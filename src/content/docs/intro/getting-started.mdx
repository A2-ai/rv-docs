...
`rv init` will create the following project structure:
...
The [`r_version` page](../../config/required-fields/r_version) contains additional information on how `rv` finds the [R executable.](../../config/required-fields/r_version/#finding-the-executable)

:::caution
If using RStudio/Positron, the R version on the path does not always match the version selected.
:::

:::tip
Managing R installations with [rig](https://github.com/r-lib/rig) makes it easy to keep multiple R versions available on the same system, so different projects can pin different versions without conflicting.
:::
...
<LinkCard title="Project Configuration" href="../../config/" />
</Aside>
...
<LinkCard title="Commands" href="../../commands" />
</Aside>
