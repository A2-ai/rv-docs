...
* `--r-version <VERSION>` - Paired with `--r-bin`, run against an R that does not match the config. The lockfile is then neither read nor updated <Badge text="Introduced in rv v0.23.0" variant="note"/>
...
See [self-contained scripts](../../../reference/self_contained_scripts) for the details of the block and what `rv`
fills in for you.
