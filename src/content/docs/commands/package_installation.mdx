...
## `rv upgrade`
`rv` will default to installing packages from the source they are locked in, as long as that source is still within the configuration file,
as discussed in the [Resolution Reference](../../reference/resolution).
