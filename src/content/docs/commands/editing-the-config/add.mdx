...
The `rv add` command supports adding packages as [simple dependencies](../../../config/required-fields/dependencies#simple) or with advanced configuration options like specifying repositories, git sources, local paths, URLs, and various installation behaviors.
...
* `--r-bin <PATH>` - Use the R at that path instead of the one `rv` finds for the project. Its version has to match the project's `r_version`, see [overriding the R version](../../../config/required-fields/r_version#overriding-the-r-version) <Badge text="Introduced in rv v0.23.0" variant="note"/>
...
(or [`git_shorthand_base_url`](../../../config/package-compilation/git_shorthand_base_url) if configured):
