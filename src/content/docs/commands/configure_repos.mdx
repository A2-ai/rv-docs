...
while maintaining the file's formatting and comments.
...
```bash
rv configure repositories --alias "PPM_latest" --url "https://packagemanager.posit.co/cran/latest" --first
```
...
```bash
rv configure repositories --alias "bioconductor" --url "https://bioconductor.org/packages/3.18/bioc" --last
```
...
```bash
rv configure repositories --alias "PPM_2024_06" --url "https://packagemanager.posit.co/cran/2024-06-01" --before PPM
```
...
```bash
rv configure repositories --alias "PPM_2024_06" --url "https://packagemanager.posit.co/cran/2024-06-01" --force_source --after CRAN
```
...
<Tabs syncKey="configure">
    <TabItem label = "Updated Config File">
        ...
        repositories = [
            { alias = "CRAN", url = "https://cran.r-project.org" },
            { alias = "PPM_2024_06", url = "https://packagemanager.posit.co/cran/2024-06-01", force_source = true },
            { alias = "PPM", url = "https://packagemanager.posit.co/cran/2025-01-01" },
        ]
        dependencies = [
            "dplyr",
...
```bash
rv configure repositories --alias "PPM_latest" --url "https://packagemanager.posit.co/cran/latest" --replace PPM
```
...
```bash
rv configure repositories --remove "CRAN"
```
...
```bash
rv configure repositories --clear
```
...
**Command:**
```bash
rv configure repositories --alias "CRAN" --url = "https://cran.rstudio.com" --first
```
...
**Command:**
```bash
rv configure repositories --alias "invalid" --url "not-a-valid-url" --first
```
