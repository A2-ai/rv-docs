...
2. Migrating your `renv.lock` file to an `rv` config file.
...
2. If the exact version is required and cannot be found in a different repository, use the [url dependency format](../../../config/required-fields/dependencies#url) to directly access the archive
...
### Options
* `--strict-r-version` - As discussed in the [config `r_version` section](../../../config/required-fields/r_version), the R version is recommended to only be listed as the major and minor
components. This flag will include the patch in the R version if needed.

* `--no-r-environment` - By default, `rv` creates a R environment `.rv` to enable `rv` to be called in R code 
...
therefore we will add this package as a [URL dependency](../../../config/required-fields/dependencies#url), pointing to the archive.
