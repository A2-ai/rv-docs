...
2. Creating a configuration file which is populated with the R version and repositories
...
* `--sandbox` - Writes `sandbox = true` into the generated config, so the project runs against a
[system library sandbox](../../../concepts/sandbox) instead of R's own library.
