...
Since `bar` is locked to `1.0.0` we start with that requirement. While we prefer `foo` to come as `2.0.0` from `repo2`, but `bar 1.0.0` fails the requirement that
it must be `>= 2.0.0`. In this case, we ignore the preference to install from the first repository to meet the lockfile requirement and install `foo 1.0.0` from `repo1`.
