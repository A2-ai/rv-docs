...
* ***Direct** Dependencies* - These are the packages explicitly stated in the [dependencies section](../../config/required-fields/dependencies)
...
`bar 2.0.0` from `repo2`, but that is in direct conflict with the constraint `bar` must be sourced from `repo1`. 
...
If a lockfile is not needed for your project, set the [`use_lockfile`](../../config/project-options/use_lockfile) config option to false.
...
Like we have discussed in [other sections](../../commands#project-status), one of the benefits of `rv` is the ability to resolve and see what will happen
...
## Remotes
In [Overriding Remotes](../../cookbook/remotes) and [`prefer_repositories_for`](../../config/package-compilation/prefer_repositories_for),
...
`rv` always respects the Remotes field of a package, unless the [`prefer_repositories_for`](../../config/package-compilation/prefer_repositories_for) configuration is specified.
...
If a remote is listed, `rv` will install the dependency from the remote, unless that dependency is listed in [`prefer_repositories_for`](../../config/package-compilation/prefer_repositories_for).
...
[`prefer_repositories_for`](../../config/package-compilation/prefer_repositories_for) section in the config file.
...
for [`prefer_repositories_for`](../../config/package-compilation/prefer_repositories_for) are met:
