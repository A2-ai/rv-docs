...
<Tabs>
    ...
    <TabItem label = "rv cache output">
        ...
        * Source subdirectories - This lists all the sources, and their corresponding subdirectories, 
        ...
        Git: https://github.com/a2-ai/osinfo, source: /Users/rv-user/.cache/rv/git/8497b6723f, binary: /Users/rv-user/.cache/rv/8497b6723f/4.5/arm64/f815095b7b
        ```
    </TabItem>
    <TabItem label = "Cache File Tree">
        ...
        installed each time. Also contained for each repository binary type is a `packages.mp` file, 
...
For additional tunability, use the [`RV_CACHE_DIR` env var](../../reference/env_vars/#rv_cache_dir).
...
The details of package caching vary based on the [dependency type](../../config/required-fields/dependencies).
...
3. Download the binary package from the repository's binary archive, if the package's version was resolved from the lockfile
   (ignored if `force_source`). This lets a lockfile-pinned version still install as a binary even after it has aged out of
   the repository's current snapshot.
4. Compile the package from the cached source package
5. Download the source package from the resolved source and compile
...
This is something to always consider when using `force_source`, [`packages_env_vars`](../../config/package-compilation/packages_env_vars), and [`configure_args`](../../config/package-compilation/configure_args)
as changing the compiled binary will affect other packages using the same package binary. 
:::
...
We keep all the sandboxes we ever create in the cache. Most of the time, they are just folders containing symlinks which are never deleted.
...
:::caution
The directory pointed to by `RV_GLOBAL_CACHE_DIR` must already exist. If it doesn't (or isn't accessible), `rv` logs a warning and
silently falls back to using only the local cache.
:::
