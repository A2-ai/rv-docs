...
import { Card, Code, FileTree } from '@astrojs/starlight/components';
...
* ***Direct** Dependencies* - These are the packages explicity stated in the [dependencies section](../../config/common_config/#dependencies)
...
* ***Recursive** Dependencies* - Most packages have other packages which it requires to work. Packages define its dependencies as requirements, 
often a package name and acceptable version constraint, and occassionally [a remote source](https://r-pkgs.org/dependencies-in-practice.html#depending-on-the-development-version-of-a-package).
...
Since `repo1` is first in the config file, we prefer packages to be sourced from that repository. Since `foo 1.0.0` and `bar 1.0.0` can be found in `repo1`
and they meet the requirements, we end up with the same resolution as Example 1: `foo 1.0.0` and `bar 1.0.0` from `repo1`
...
But in this case, we give preference to `repo2`. Since `foo 2.0.0` and `bar 2.0.0` are in `repo2` and they meet the requirements, 
the resolution is `foo 2.0.0` and `bar 2.0.0` from `repo2`.

#### Example 2.2 - Specific Repositories

##### Example 2.2.1 - Resolvable
Lets return to [Example 2.1.1](#example-211---repo1-first) where `repo1` is first, but this time ensuring I get `foo 2.0.0` from `repo2`:
```toml {7}
//rproject.toml
repositories = [ 
    { alias = "repo1", url = "https://cran-like-repo.com/repo1" },
...
Since `foo 2.0.0` is in `repo2`, we must also meet the requirement of `bar >= 2.0.0`, therefore we override the preference that packages come from `repo1`
and `bar 2.0.0` is install from `repo2`. Thus, the resolution ends up being the same as if `repo2` was first: `foo 2.0.0` and `bar 2.0.0` from `repo2`.
...
Since `foo 2.0.0` is in `repo2` and `bar 1.0.0` is in `repo1`, but we have the requirement that `bar >= 2.0.0` from `foo 2.0.0`, we cannot resolve
this configuration.
...
Lets consider the same package and repository information as [Example 2](#example-2---two-repositories):
<Card title = "Package and Repository Information">
    ###### Repositories Content
    <FileTree>
    - repo1
...
For this example, we will have installed `bar` from `repo1`, generating the following lockfile:
...
##### Example 3.3.1 - Not Compatible Locked Version
Lets return to [Example 3.1](#example-31---new-repository) and add `foo` to the dependencies field.
...
Like we have discussed in [other sections](../../commands/project_status), one of the benefits is `rv` is the ability to resolve and see what will happen
before it actually occurs. If ran an `rv plan` and saw the resolution from the [above example](#example-331---not-compatible-locked-version) was not the desired
outcome, we can change our config file to ensure we get the same version of `bar` as is locked.
...
## Remotes
In [Overriding Remotes](../../cookbook/remotes) and [`prefer_repositories_for`](../../config/additional_config/#prefer_repositories_for),
we have discussed Remotes resolution, but we'll discuss further in this section. 

Within a packages DESCRIPTION file, the Remotes field can be set to install a dependencies from a nonstandard place. This often occurs when developing one package
alongside a development version of another. This is dicussed further in the [R package book](https://r-pkgs.org/dependencies-in-practice.html#sec-dependencies-nonstandard).

`rv` always respects the Remotes field of a package, unless the [`prefer_repositories_for`](../../config/additional_config/#prefer_repositories_for) configuration is specified.
...
* `baz 1.0.0.9000` - in development on the main branch of `my-org/baz` 
    ...
    By not indicating a reference in the Remote, `rv` (and the default tooling) installs from the HEAD, which is main in this example.
...
The previous example was did not have multiple resolutions since `bar 2.0.0` from `repo2` did not meet the requirement set by the `baz 1.0.0.9000`,
...
Lets we keep the same configuration file as [Example 4.1](#example-41---installing-from-remotes), but with `repo3` instead:
...
If a remote is listed, `rv` will install the dependency from the remote, unless that dependency is listed in [`prefer_repositories_for`](../../config/additional_config/#prefer_repositories_for).
...
[`prefer_repositories_for`](../../config/additional_config/#prefer_repositories_for) section in the config file.
...
for [`prefer_repositories_for`](../../config/additional_config/#prefer_repositories_for) are met:
