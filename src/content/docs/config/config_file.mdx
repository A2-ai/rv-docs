...
description: The configurations available for package installation, most commonly used by 
---

import { Aside, Code, Card, Badge } from '@astrojs/starlight/components';

`rv` was designed to provide a simple, reproducible way to manage packages, while still being powerful and flexible enough to
fit the most demanding needs. We will first go over the different configuration options that are most commonly used.
...
<Aside type="note">
R packages installed across patch versions are compatible, 
...
* `force_source` - Force all packages coming from the repository to be source. <Badge text="optional" variant="caution"/>
...
* `install_suggestions`: Install the suggested dependencies of the package <Badge text="optional" variant="caution"/>
* `dependencies_only`: Install only the dependencies of the package, not the package itself. Used primarily for development. <Badge text="optional" variant="caution"/>
...
##### Configurations:
* `name` - The name of the package. Required unless no other option is specified.
* `repository` - Used to specify that a package must come from a specific repository, 
by setting to the alias corresponding alias from the `repositories` section. <Badge text="optional" variant="caution"/>
* `force_source` - Force the package to be fetched as source. <Badge text="optional" variant="caution"/>

```toml
dependencies = [
    "dplyr", # a simple string is equivalent to `{ name = "dplyr" }`
    { name = "purrr", force_source = true, force_source = true },
...
* `path` - a path to a subdirectory inside of the repository containing the R package <Badge text="optional" variant="caution"/>
```toml
dependencies = [
    ...
    { name = "git-branch", git = "https://github.com/a2-ai/git-branch", branch = "main" },
]
```
