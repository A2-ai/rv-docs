...
* `force_source` <Badge text="optional" variant="caution"/> - Force the package to be fetched as source
    > Default: `false`
* `bioconductor` <Badge text="optional" variant="caution"/> <Badge text="rv v0.23.0" variant="note"/> - Only look for the package
in the repositories expanded from the [`bioconductor` shorthand](../repositories#bioconductor). Requires that shorthand
to be present and cannot be combined with `repository`
    > Default: `false`
...
dependencies = [
    ...
    { name = "Biobase", bioconductor = true },
]
```
...
```toml
dependencies = [
    { name = "pkg1", git = "https://github.com/org/pkg1", tag = "v1.0.0" },
    { name = "pkg2", git = "https://github.com/org/pkg2", commit = "bc50e550e432c3c620714f30dd59115801f89995" },
    { name = "pkg3", git = "https://github.com/org/pkg3", branch = "main", directory = "r" },
]
```
...
:::note
Starting in v0.21.0, locally sourced packages are built using `R CMD build` before installation.
:::
